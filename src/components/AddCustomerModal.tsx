...
export default function AddCustomerModal({ onClose, onSaved, customer }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-lg bg-white rounded-t-3xl shadow-drawer p-6 pb-8 max-h-[90dvh] overflow-y-auto"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">Full Name *</label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">Phone Number *</label>
            <input
              type="tel"
              required
          ...
          {isEditing ? (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">Completed Classes</label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-11 px-3 rounded-xl bg-slate-50 text-slate-300 text-[14px] font-semibold cursor-not-allowed select-none border border-slate-100 opacity-80"
                    />
                    <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-[16px] text-slate-300">lock</span>
                  </div>
                </div>
                <div>
                  <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">Package Classes</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">Total Fee (₹) *</label>
                <input
                  type="number"
                  min="0"
                  required
          ...
          <div>
            <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">Enrollment Date</label>
            <input
              type="date"
          ...
          <div>
            <label className="text-[11px] text-slate-500 block mb-1 uppercase tracking-wider">
              Location (Pickup / Drop)
            </label>
            <div className="relative">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[18px] text-slate-300 pointer-events-none">location_on</span>
              <input
                type="text"
                value={form.location}
                onChange={e => set('location', e.target.value)}
                placeholder="Eg. Sector 15, Noida"
                className="w-full h-11 pl-9 pr-3 rounded-xl bg-slate-50 text-slate-900 text-[14px] focus:outline-none focus:bg-white transition-all placeholder:text-slate-300"
              />
            </div>
          </div>
