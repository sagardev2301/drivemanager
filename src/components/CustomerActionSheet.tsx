      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-surface-container-low">
            <div className="w-12 h-12 rounded-full bg-primary text-on-primary flex items-center justify-center text-[16px] font-bold shrink-0">
              {getInitials(customer.full_name)}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[16px] font-semibold text-on-surface truncate">{customer.full_name}</p>
              <p className="text-[12px] text-on-surface-variant">{customer.phone_number}</p>
            </div>
            {hasPending ? (
              <span className="px-2.5 py-1 rounded-full bg-error-container text-error text-[12px] font-semibold shrink-0">
                Rs.{customer.amount_pending.toLocaleString('en-IN')} Pending
              </span>
            ) : (
              <span className="px-2.5 py-1 rounded-full bg-tertiary-fixed/30 text-tertiary text-[12px] font-semibold shrink-0">
                Paid
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 px-5 pt-4">
            <button
              ...
              disabled={!hasPending}
              className={'w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-left transition-all active:scale-[0.98] ' + (hasPending ? 'bg-primary text-on-primary shadow-sm' : 'bg-surface-container-low text-outline opacity-60 cursor-not-allowed')}
            >
            ...
            <button
              ...
              className={'w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-left transition-all active:scale-[0.98] ' + (hasPhone && hasPending ? 'bg-tertiary-fixed/30 text-tertiary' : 'bg-surface-container-low text-outline opacity-60 cursor-not-allowed')}
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl bg-surface-container-low text-on-surface text-left transition-all active:scale-[0.98]"
            >
