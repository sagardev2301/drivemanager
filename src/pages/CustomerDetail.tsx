    ...
    <div className="flex flex-col w-full pb-8">
            ...
            {classes.map((cls, i) => (
              <div key={cls.id} className="flex items-center justify-between p-2.5 rounded-lg bg-[#f1f3ff]">
                <div className="flex items-center gap-2.5">
                  <div className="w-6 h-6 rounded-full bg-[#e1e8fd] text-[#003fb1] text-[11px] font-bold flex items-center justify-center">
                    {classes.length - i}
                  </div>
                  <div>
                    <span className="text-[13px] font-semibold text-[#141b2b]">{formatDate(cls.class_date)}</span>
                    {cls.start_time && (
                      <span className="text-[11px] text-[#434654] ml-2">{formatTime(cls.start_time)}</span>
                    )}
                  </div>
                </div>
                <div className="flex items-baseline gap-1 mb-2">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[12px] font-semibold text-[#005623]">
            ₹{customer.amount_paid.toLocaleString('en-IN')} of ₹{customer.total_fee.toLocaleString('en-IN')}
          </span>
        </div>
        {payments.length === 0 ? (
          <p className="text-[13px] text-[#434654] text-center py-2">No payments recorded yet</p>
        ) : (
          <div className="flex flex-col gap-2">
            {payments.map(p => (
              <div key={p.id} className="flex items-center justify-between p-2.5 rounded-lg bg-[#f1f3ff]">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg bg-[#6bff8f]/30 text-[#005623] flex items-center justify-center">
                    <span className="material-symbols-outlined text-[18px]">payments</span>
                  </div>
                  <div>
                    <span className="text-[14px] font-bold text-[#141b2b]">₹{p.amount.toLocaleString('en-IN')}</span>
                    <p className="text-[11px] text-[#434654]">{formatDate(p.created_at)} • {payModeLabel[p.payment_mode] ?? p.payment_mode}</p>
                  </div>
                </div>
                <p className="text-[11px] text-[#434654] font-medium">{customer.classes_remaining} Remaining</p>
                <span className="text-[11px] font-semibold text-[#005623] bg-[#6bff8f]/30 px-2 py-0.5 rounded-full">Received</span>
              </div>
            </div>
            ))}
          </div>
        ...
        {customer.amount_pending > 0 && (
          <div className="mt-3 p-3 rounded-xl bg-[#b5c4ff]/20 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[18px] text-[#1a3f9c]">account_balance_wallet</span>
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[11px] text-[#434654] uppercase tracking-wider">Balance</span>
                  <span className="material-symbols-outlined text-[18px] text-[#3858b6]">payments</span>
